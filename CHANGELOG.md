...
### Transfers（传输）
- **前后台分离**：浏览大文件夹时的缩略图下载不再计入状态栏进度条、传输队列、Dock 徽标与完成通知；进度条只在用户传输进行中显示，完成后立即收起
- **取消不算失败**：用户暂停/取消的传输不再触发「N 个文件 failed」错误横幅；批量 / 目录下载与上传的失败汇总与用户取消正确区分
- **目录下载完整性**：任一子目录列表失败即整目录下载失败，不再静默交付缺文件的「成功」文件夹；本地目录不可读 / 远端 mkdir 失败会如实上抛
- **单项操作可见性**：单文件另存为下载、⌘K 上传、预览下载失败时显示错误横幅，不再无声无息
- **传输历史持久化**：按设备序列保存到磁盘、重启后恢复；原子写入 + 损坏文件自动备份
- 文件夹大小统计（≤2000 项、10 分钟缓存、可取消）；批量失败摘要横幅（列出前 3 个文件名）
- ADB shell 调用（按键注入、开发者设置跳转）增加 5 秒硬超时；AdbRunner 改用信号量等待，不再空转 CPU

### Thumbnails（缩略图）
- 图像缩略图走 ImageIO ≤512px 快速解码（HEIC/JPEG 内嵌缩略图直接复用），视频取 1s 帧（失败回退首帧），PDF 取首页
- 缓存身份含路径哈希 + 大小 + 修改时间；损坏缩略图 / 原图自动删除并重新渲染；同文件并发去重、前台传输优先

### UI
- 面包屑：深层路径自动滚动使当前文件夹完整可见，长段名中间截断防撑爆胶囊
- 检查器：宽度锁定滚动条 gutter、缩略图固定高度舞台、快速切换选择时的竞态守卫
- 命令面板（⌘K）：键盘 ↑↓ 导航时选中项自动滚入视野，鼠标悬停不拽动列表
- 新增快捷键：**⌘1 / ⌘2** 列表/网格切换、**⇧⌘.** 显示隐藏文件、**⌘J** 传输队列
- App 列表长名称中间截断；侧边栏电池 / 存储信息在偶发 adb 失败时保留上次数值，不再闪烁
